...
| **订阅导出** | 普通订阅链接、Clash/Mihomo（含 AnyTLS 等） | API `sub` 与订阅保护逻辑 |
...
- **可维护性**：业务高度集中在单文件 API 与单页 HTML；改动面大，回归应覆盖登录、VPS 上报、订阅导出、实时连接四条主路径。
...
说明：本 spec 首版从 README + 入口代码 + schema/路由抽样归纳；细到每个 action 的请求体字段未逐条展开。改某一 action 前应再读对应代码分支，必要时补子层 spec 或 Explore issue。
